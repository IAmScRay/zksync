...
class Mute(Module):
    ...
    def __init__(
        ...
        self.contract = self.get_provider().eth.contract(
            address=config.get_contract_address("Mute"),
            abi=config.get_contract_abi("Mute")
        )

        self.o_id = 0

    def get_id(self) -> int:
        return self.get_config().get_project_id("Mute")

    def run(self):
        rand_op = random.randint(1, 2)
        if rand_op == 1:
            print(f"[Mute] Адрес {self.get_account().address}: будет проведена swap-операция ETH -> USDC")
            self.o_id = utils.SWAP_ETH_USDC
        elif rand_op == 2:
            print(f"[Mute] Адрес {self.get_account().address}: будет проведена swap-операция USDC -> ETH")
        ...
        if self.o_id == utils.SWAP_ETH_USDC:
            ...
            if current_gas_price > self.get_config().get_max_gwei():
                print(f"[Mute] Адрес {self.get_account().address}: слишком высокая цена за газ!\n"
            ...
            while gas == 0:
                try:
                    gas = self.contract.functions.swapExactETHForTokensSupportingFeeOnTransferTokens(
                        min_out,
                        [
                    ...
                    print(f"[Mute] Адрес {self.get_account().address}: ошибка при определении лимита газа!\n"
            ...
            swap_tx = self.contract.functions.swapExactETHForTokensSupportingFeeOnTransferTokens(
                min_out,
                [
                    self.get_config().get_token_address("WETH"),
                    self.get_config().get_token_address("USDC")
            ...
            if not self.has_enough_allowance(
                ...
                if self.approve(
                    ...
                    while gas == 0:
                        try:
                            gas = self.contract.functions.swapExactTokensForETHSupportingFeeOnTransferTokens(
                                portion,
                                min_out,
                                [
                                    self.get_config().get_token_address("USDC"),
                    ...
                    swap_tx = self.contract.functions.swapExactTokensForETHSupportingFeeOnTransferTokens(
                        portion,
                        min_out,
                        [
                            self.get_config().get_token_address("USDC"),
    ...
    def get_min_out(
        ...
        return int(amount_out[0] - amount_out[0] / 100 * self.get_config().get_slippage("Mute"))
