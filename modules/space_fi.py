...
class SpaceFi(Module):
    ...
    def __init__(
        ...
        self.contract = self.get_provider().eth.contract(
            address=config.get_contract_address("SpaceFi"),
            abi=config.get_contract_abi("SpaceFi")
        )

        self.o_id = 0

    def get_id(self) -> int:
        return self.get_config().get_project_id("SpaceFi")

    def run(self):
        rand_op = random.randint(1, 2)
        if rand_op == 1:
            print(f"[SpaceFi] Адрес {self.get_account().address}: будет проведена swap-операция ETH -> USDC")
            self.o_id = utils.SWAP_ETH_USDC
        elif rand_op == 2:
            print(f"[SpaceFi] Адрес {self.get_account().address}: будет проведена swap-операция USDC -> ETH")
        ...
        if self.o_id == utils.SWAP_ETH_USDC:
            ...
            while gas == 0:
                try:
                    ...
                    print(f"[SpaceFi] Адрес {self.get_account().address}: ошибка при определении лимита газа!\n"
            ...
            current_gas_price = utils.get_gas_gwei()
            if current_gas_price > self.get_config().get_max_gwei():
                print(f"[SpaceFi] Адрес {self.get_account().address}: слишком высокая цена за газ!\n"
                      f"• требуется цена {self.get_config().get_max_gwei()} GWei и ниже\n"
                      f"• текущая цена – {current_gas_price} GWei\n")
                return
    ...
    def get_min_out(
        ...
        return int(amount_out[1] - amount_out[1] / 100 * self.get_config().get_slippage("SpaceFi"))
